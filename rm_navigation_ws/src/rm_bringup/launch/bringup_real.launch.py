...
def generate_launch_description():
    ...
    urdf_dir = os.path.join(rm_bringup_dir, 'urdf', 'sentry_robot.xacro')
    ...
    segmentation_params = os.path.join(rm_bringup_dir, 'config', 'reality', 'segmentation_real.yaml')
    ########################## linefit_ground_segementation parameters end ############################

    #################################### FAST_LIO parameters start ####################################
    fastlio_mid360_params = os.path.join(rm_bringup_dir, 'config', 'reality', 'fastlio_mid360_real.yaml')
    fastlio_rviz_cfg_dir = os.path.join(rm_bringup_dir, 'rviz', 'fastlio.rviz')
    ##################################### FAST_LIO parameters end #####################################

    ################################### POINT_LIO parameters start ####################################
    pointlio_mid360_params = os.path.join(rm_bringup_dir, 'config', 'reality', 'pointlio_mid360_real.yaml')
    pointlio_rviz_cfg_dir = os.path.join(rm_bringup_dir, 'rviz', 'pointlio.rviz')
    #################################### POINT_LIO parameters end #####################################

    ################################## slam_toolbox parameters start ##################################
    slam_toolbox_map_dir = PathJoinSubstitution([rm_bringup_dir, 'map', world])
    slam_toolbox_localization_file_dir = os.path.join(rm_bringup_dir, 'config', 'reality', 'mapper_params_localization_real.yaml')
    slam_toolbox_mapping_file_dir = os.path.join(rm_bringup_dir, 'config', 'reality', 'mapper_params_online_async_real.yaml')
    ################################### slam_toolbox parameters end ###################################

    ################################### navigation2 parameters start ##################################
    nav2_map_dir = PathJoinSubstitution([rm_bringup_dir, 'map', world]), ".yaml"
    nav2_params_file_dir = os.path.join(rm_bringup_dir, 'config', 'reality', 'nav2_params_real.yaml')
    ################################### navigation2 parameters end ####################################

    ################################ icp_registration parameters start ################################
    icp_pcd_dir = PathJoinSubstitution([rm_bringup_dir, 'PCD', world]), ".pcd"
    icp_registration_params_dir = os.path.join(rm_bringup_dir, 'config', 'simulation', 'icp_registration_sim.yaml')
    ################################# icp_registration parameters end #################################

    ############################# pointcloud_downsampling parameters start ############################
    pointcloud_downsampling_config_dir = os.path.join(rm_bringup_dir, 'config', 'reality', 'pointcloud_downsampling_real.yaml')
    ############################# pointcloud_downsampling parameters start ############################

...
def generate_launch_description():
    ...
    segmentation_params = os.path.join(rm_bringup_dir, 'config', 'simulation', 'segmentation_sim.yaml')
    ########################## linefit_ground_segementation parameters end ############################

    #################################### FAST_LIO parameters start ####################################
    fastlio_mid360_params = os.path.join(rm_bringup_dir, 'config', 'simulation', 'fastlio_mid360_sim.yaml')
    fastlio_rviz_cfg_dir = os.path.join(rm_bringup_dir, 'rviz', 'fastlio.rviz')
    ##################################### FAST_LIO parameters end #####################################

    ################################### POINT_LIO parameters start ####################################
    pointlio_mid360_params = os.path.join(rm_bringup_dir, 'config', 'simulation', 'pointlio_mid360_sim.yaml')
    ...
    pointcloud_downsampling_config_dir = os.path.join(rm_bringup_dir, 'config', 'simulation', 'pointcloud_downsampling_sim.yaml')
    ############################# pointcloud_downsampling parameters start ############################
    ...
    bringup_LIO_group = GroupAction([
        Node(
            ...
            # Copy from the 'livox_joint' in 'simulation_walking_robot.xacro'.
            arguments=[
                '--x', '0.12',
                '--y', '0.0',
                '--z', '0.175',
